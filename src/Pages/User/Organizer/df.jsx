...
const MAX_IMAGES = 5
const API_BASE_URL = "https://localhost:7255" // or your actual API base URL
...
const EditEvent = () => {
  ...
  const [confirmActionType, setConfirmActionType] = useState(null) // 'draft' or 'publish'
...
useEffect(() => {
  ...
  // Helper to extract comparable image info
  const normalizeFormData = (data) => {
    const safeDate = (val) => {
      if (!val) return ""
      try {
        const d = typeof val === "string" ? new Date(val) : val
        return isNaN(d?.getTime()) ? "" : d.toISOString().split("T")[0] // Compare only dates
      } catch {
        return ""
      }
    }
    ...
    return {
      ...
      registrationExpiry: safeDate(data.registrationExpiry),
      ticketPrice: data.ticketType === "paid" ? Number.parseFloat(data.ticketPrice || 0) : 0,
    }
  }
  ...
  const extractImageInfo = (files) =>
    files
      .map((f) => ({
        id: f.id || null, // Add ID for existing files
  ...
  const extractVideoInfo = (video) =>
    video
      ? {
          id: video.id || null, // Add ID for existing files
          name: video.name || video.file?.name || "",
          size: video.size || video.file?.size || 0,
          type: video.type || video.file?.type || "",
  ...
  // Update the loadEventData function in EditEvent.jsx
  const loadEventData = async () => {
    try {
      ...
      if (!eventResponse || !eventResponse[0]) {
        throw new Error("Event data not found")
      }

      const eventData = eventResponse[0]
      ...
      // Fetch event media
      const mediaResponse = await eventService.getEventMedia(eventId)

      // Handle registration_expiry parsing
      let registrationExpiry = null
      if (eventData.registration_expiry) {
        if (eventData.registration_expiry instanceof Date) {
          registrationExpiry = eventData.registration_expiry
        } else if (typeof eventData.registration_expiry === "string") {
          registrationExpiry = new Date(eventData.registration_expiry)
        } else if (eventData.registration_expiry.date) {
          registrationExpiry = new Date(eventData.registration_expiry.date)
        }

        if (isNaN(registrationExpiry?.getTime())) {
          registrationExpiry = null
        }
      }

      // Process existing media
      const existingImages = mediaResponse.images || []
      const existingVideo = mediaResponse.videos?.[0] || null
      ...
      const imagePreviews = existingImages.map((img) => ({
        id: img.id.toString(),
        preview: `${API_BASE_URL}/${img.file_path}`,
        name: img.name,
        type: img.type,
        size: img.size,
        uploaded: true,
      ...
      // Create preview for existing video if exists
      const videoPreview = existingVideo
        ? {
            id: existingVideo.id.toString(),
            preview: `${API_BASE_URL}/${existingVideo.file_path}`,
      ...
      setSaveStatus(eventData.status || "draft")
  ...
  const onImageDrop = useCallback((acceptedFiles) => {
    setImageFiles((prev) => {
      const existingFileNames = new Set(prev.map((file) => file.name || file.file?.name))
      const duplicateFiles = []

      const newFilesWithPreview = acceptedFiles
        .filter((newFile) => {
          const isDuplicate = existingFileNames.has(newFile.name)
          if (isDuplicate) {
      ...
      if (duplicateFiles.length > 0) {
        showSnackbar(`Skipped ${duplicateFiles.length} duplicate file(s): ${duplicateFiles.join(", ")}`, "warning")
      }

      // Show success if new files were added
      if (newFilesWithPreview.length > 0) {
        showSnackbar(`Added ${newFilesWithPreview.length} new image(s)`, "success")
      }
      ...
      if (combinedFiles.length > MAX_IMAGES) {
        ...
        const removedFiles = combinedFiles.slice(0, filesToRemove)
        showSnackbar(`Maximum ${MAX_IMAGES} images allowed. Removed ${filesToRemove} image(s)`, "warning")
  ...
  //track changes in form data, images, and video
  useEffect(() => {
    if (!initialDataLoaded || !originalDataRef.current) return

    const currentForm = normalizeFormData(formData)
    const originalForm = normalizeFormData(originalDataRef.current.formData)

    const currentImageInfo = extractImageInfo(imageFiles)
    const originalImageInfo = extractImageInfo(originalDataRef.current.imageInfo)

    const currentVideoInfo = extractVideoInfo(videoFile)
    const originalVideoInfo = extractVideoInfo(originalDataRef.current.videoInfo)

    const hasFormChanged = JSON.stringify(currentForm) !== JSON.stringify(originalForm)
    const hasImageChanged = JSON.stringify(currentImageInfo) !== JSON.stringify(originalImageInfo)
    const hasVideoChanged = JSON.stringify(currentVideoInfo) !== JSON.stringify(originalVideoInfo)

    const hasChanged = hasFormChanged || hasImageChanged || hasVideoChanged
    // if (hasFormChanged) {
    //   console.log("🔍 Form diff:");
    //   console.log("Original:", normalizeFormData(originalDataRef.current.formData));
    //   console.log("Current :", normalizeFormData(formData));
    // }

    setIsChanged(hasChanged)

    // console.log('Changed?', hasChanged, { hasFormChanged, hasImageChanged, hasVideoChanged });
  }, [formData, imageFiles, videoFile, initialDataLoaded])
    ...
    <div className="border-2 border-dashed rounded-lg p-6 mb-4">
      <div
        ...
        className={`p-8 text-center cursor-pointer transition-colors ${
          isImageDragActive ? "border-indigo-500 bg-indigo-50" : "border-gray-300 hover:border-gray-400"
        }`}
      >
  ...
  const renderVideoUploadSection = () => (
    <div className="border-2 border-dashed rounded-lg p-6 mb-4">
      <div
        {...getVideoRootProps()}
        className={`p-8 text-center cursor-pointer transition-colors ${
          isVideoDragActive ? "border-indigo-500 bg-indigo-50" : "border-gray-300 hover:border-gray-400"
        }`}
      >
  ...
  const handleActionWithConfirmation = (actionType) => {
    setConfirmActionType(actionType) // 'draft' or 'publish'
  ...
  const handleConfirmedAction = async () => {
    ...
    try {
      if (confirmActionType === "draft" && isChanged) {
        if (!validateAllSteps()) {
          showSnackbar("Please fix validation errors before saving", "warning")
          return
        }
        ...
        const filesToUpload = imageFiles.filter((f) => !f.uploaded).map((f) => f.file)
        if (filesToUpload.length > 0) await eventService.uploadImages(eventId, filesToUpload)

        if (videoFile && !videoFile.uploaded) {
          const videoToUpload = videoFile.file || videoFile
          if (videoToUpload instanceof File) {
            await eventService.uploadVideo(eventId, videoToUpload)
          }
        }

        // Handle media deletion
        if (mediaToDelete.images.length > 0) {
          console.log("Deleting images:", mediaToDelete.images)
          await Promise.all(mediaToDelete.images.map((id) => eventService.deleteImage(id)))
          setMediaToDelete((prev) => ({ ...prev, images: [] }))
        }
        if (mediaToDelete.video) {
          await eventService.deleteVideo(mediaToDelete.video)
          setMediaToDelete((prev) => ({ ...prev, video: null }))
        }
        setSaveStatus("draft")
        showSnackbar("Event saved as draft.", "success")
      } else if (confirmActionType === "draft" && !isChanged) {
        await eventService.draftEvent(eventId)
        setSaveStatus("draft")
        showSnackbar("Event drafted successfully!", "success")
      } else if (confirmActionType === "publish" && isChanged) {
        // First validate all steps
        if (!validateAllSteps()) {
          showSnackbar("Please fix all errors before publishing.", "warning")
          return
        }
        ...
        setSaveStatus("published")
        showSnackbar("Event published successfully!", "success")
      } else if (confirmActionType === "saveChanges") {
        if (!validateAllSteps()) {
          showSnackbar("Please fix validation errors before saving", "warning")
          return
        }

        const eventData = prepareEventData(saveStatus)
        await eventService.updateEvent(eventId, eventData)
      ...
      loadEventData() // Reload event data to reflect changes
    }
  }
  ...
  // Modified media handling functions to track existing vs new media
  // const handleRemoveImage = (id) => {
  //   // Check if this is an existing image
  //   const existingImage = existingMedia.images.find(img => img.id === id);
  //   if (existingImage) {
  ...
  const handleRemoveImage = (id) => {
    setImageFiles((prevFiles) => {
      ...
      showSnackbar(`Removed image: ${fileToRemove.name || "Untitled"}`, "info")

      // If it's an existing uploaded file, mark for server deletion
      if (fileToRemove.uploaded) {
        setMediaToDelete((prev) => ({
          ...prev,
          images: [...prev.images.filter((imgId) => imgId !== id), String(id)], // Only add if not already present
        }))
      }
      ...
      if (fileToRemove.preview) {
        ...
        filesToCleanup.current.delete(fileToRemove.preview)
      }

      // Return the filtered array without the removed image
  ...
  // Add all your existing UI rendering functions from CreateEvent.jsx
  // (renderImageUploadSection, renderVideoUploadSection, renderPreviewModal, etc.)
  ...
  const prepareEventData = (status) => {
    ...
    // Handle registration expiry - send empty string if null
    if (formData.registrationExpiry instanceof Date && !isNaN(formData.registrationExpiry)) {
    ...
    <div className="flex flex-col items-end space-y-3">
      ...
      <motion.button
        ...
        className={`p-4 rounded-full shadow-lg flex items-center justify-center ${
          fabExpanded ? "bg-indigo-700" : "bg-indigo-600"
        } text-white`}
      >
          ...
          >
            ...
            {isChanged && (
              <motion.button
                ...
                whileTap={{ scale: 0.95 }}
                onClick={() => {
                  handleActionWithConfirmation("saveChanges")
                  setFabExpanded(false)
                }}
                className="p-3 bg-emerald-500 rounded-full shadow-md flex items-center justify-center text-white"
                title="Save Changes"
              >
                <FiSave size={20} />
              </motion.button>
          ...
          <div className="overflow-y-auto flex-1">
            ...
            <div className="p-6 md:p-8">
              <div className="max-w-4xl mx-auto">
                {/* Event Title */}
                <h1 className="text-3xl md:text-4xl font-bold mb-6 text-gray-900">{formData.title || "Event Title"}</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      ...
                      {formData.location && (
                        <button className="text-indigo-600 text-sm font-medium mt-1 flex items-center">
                          View on map
                ...
                <div className="mb-10">
                  ...
                  <div
                    className={`prose max-w-none break-words text-sm sm:text-base ${
                      !formData.description ? "text-gray-400 italic" : "text-gray-700"
                    }`}
                  >
  ...
  if (isLoading) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-white z-50">
        <div className="text-center">
          <div className="h-16 w-16 rounded-full border-t-4 border-b-4 border-indigo-600 animate-spin mx-auto"></div>
          <p className="mt-4 text-lg text-gray-700 font-medium">Loading event data...</p>
        </div>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto p-4 md:p-6 lg:p-8 bg-white rounded-xl shadow-sm">
            ...
            <div className="flex items-center space-x-2 text-gray-600">
              ...
              <span className="text-sm">
                {formData.registrationExpiry && !isNaN(new Date(formData.registrationExpiry))
                  ? `Registration closes: ${new Date(formData.registrationExpiry).toLocaleDateString()}`
            ...
            <div className="flex items-center justify-between">
              <span
                className={`px-2.5 py-1 text-xs rounded-full font-medium ${
                  saveStatus === "draft" ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"
                }`}
              >
          ...
          <div className="space-y-3 bg-white rounded-xl border border-gray-200 p-5 shadow-sm">
            ...
            {saveStatus === "published" && (
              <button
                ...
                disabled={isSaving}
                className="w-full py-2.5 px-4 rounded-lg flex items-center justify-center bg-amber-500 text-white hover:bg-amber-600 disabled:opacity-70 disabled:cursor-not-allowed transition-colors shadow-sm"
              >
                {isSaving ? (
                  <>
                    <svg
            ...
            {saveStatus !== "published" && (
              <button
                ...
                disabled={isSaving}
                className="w-full py-2.5 px-4 rounded-lg flex items-center justify-center bg-emerald-600 text-white hover:bg-emerald-700 disabled:opacity-70 disabled:cursor-not-allowed transition-colors shadow-sm"
              >
                {isSaving ? (
                  <>
                    <svg
                  ...
                  <>
                    <svg className="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        ...
                        strokeLinejoin="round"
                        strokeWidth="2"
                        d="M8 7V5a2 2 0 012-2h4a2 2 0 012 2v2m0 8v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-4m8 0H6m12 0a2 2 0 012 2v4a2 2 0 01-2 2h-4a2 2 0 01-2-2v-4"
                      />
                    </svg>
                    Publish Event
                  </>
            ...
            {isChanged && (
              <button
                onClick={() => handleActionWithConfirmation("saveChanges")}
                disabled={isSaving}
            ...
            {saveStatus === "draft" && (
              <button
                ...
                disabled={isSaving || saveStatus === "published"}
          ...
          <AnimatePresence mode="wait">
            ...
            >
                              ...
                              {imageFiles.map((file, index) => (
                                <div key={file.id} className="relative aspect-square group">
                                  ...
                                  <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-2 rounded-b-lg">
                                    <p className="text-white text-xs truncate">{file.name || `Image ${index + 1}`}</p>
                                  </div>
                                </div>
                              ))}
                          ...
                          <div className="mb-6">
                            ...
                            <div className="relative group">
                              ...
                              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-3 rounded-b-lg">
                                <p className="text-white text-sm truncate">{videoFile.name || "Event Video"}</p>
                              </div>
                            </div>
                          </div>
                        )}
                    ...
                    <div className="mb-6">
                      ...
                      <div className="relative">
                        <textarea
                          ...
                          className={`w-full p-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 h-24 transition-colors ${
                            errors.summary && touched.summary ? "border-red-500 bg-red-50" : "border-gray-300"
                          }`}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center mb-2">
                          <div
                            className={`w-5 h-5 rounded-full border flex items-center justify-center mr-2 ${
                              formData.visibility === "public" ? "border-indigo-600 bg-indigo-600" : "border-gray-400"
                            }`}
                          >
                            {formData.visibility === "public" && (
                      ...
                      >
                        <div className="flex items-center mb-2">
                          <div
                            className={`w-5 h-5 rounded-full border flex items-center justify-center mr-2 ${
                              formData.visibility === "private" ? "border-indigo-600 bg-indigo-600" : "border-gray-400"
                            }`}
                          >
                            {formData.visibility === "private" && (
                              <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className={`w-full p-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 h-64 ${
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    >
                      <div className="flex items-center mb-2">
                        <div
                          className={`w-5 h-5 rounded-full border flex items-center justify-center mr-2 ${
                            formData.ticketType === "free" ? "border-indigo-600 bg-indigo-600" : "border-gray-400"
                          }`}
                        >
                          {formData.ticketType === "free" && (
                            <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    >
                      <div className="flex items-center mb-2">
                        <div
                          className={`w-5 h-5 rounded-full border flex items-center justify-center mr-2 ${
                            formData.ticketType === "paid" ? "border-indigo-600 bg-indigo-600" : "border-gray-400"
                          }`}
                        >
                          {formData.ticketType === "paid" && (
                            <svg className="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="mt-6 p-5 bg-indigo-50 rounded-lg border border-indigo-100">
                      ...
                      <div className="relative">
                        ...
                        <input
                          type="number"
                          placeholder="0.00"
                          className={`w-full pl-8 p-3 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 ${
                            errors.ticketPrice ? "border-red-500 bg-red-50" : "border-gray-300"
                          }`}
              ...
              {(currentStep === steps.length || currentStep === steps.length - 1) && (
                <>
                  {publishStep && saveStatus === "draft" ? (
                    <div className="space-y-6">
                      <h2 className="text-xl font-bold text-gray-900 mb-4">Ready to Save</h2>
                      <div className="bg-indigo-50 p-6 rounded-lg border border-indigo-100 space-y-4">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-white p-5 rounded-lg border border-indigo-100">
                          ...
                          <div className="sm:col-span-2">
                            ...
                            <div
                              className={`prose max-w-none break-words text-base ${
                                !formData.description ? "text-gray-400 italic" : "text-gray-900"
                              }`}
                            >
                              {formData.description ? (
                                <ReactMarkdown>{formData.description}</ReactMarkdown>
                        ...
                        <div className="mt-6 p-4 bg-emerald-50 rounded-lg border border-emerald-200">
                          ...
                          <p className="text-emerald-700 text-sm">
                            Click "Save as Draft" to save your event or "Publish" to make it live.
                          </p>
                        </div>
                      </div>
                    </div>
      ...
      <Dialog open={confirmDeleteOpen} onClose={() => setConfirmDeleteOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button onClick={confirmDeleteEvent} color="error" variant="contained" autoFocus>
            Delete
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={confirmDialogOpen} onClose={() => setConfirmDialogOpen(false)}>
        <DialogTitle>{confirmActionType === "publish" ? "Confirm Publish" : "Confirm Save as Draft"}</DialogTitle>
        <DialogContent>
          {confirmActionType === "publish"
            ? "Are you sure you want to publish this event? Once published, it will be visible to the public."
            : confirmActionType === "saveChanges"
              ? "Are you sure you want to save your changes to this event?"
        ...
        <DialogActions>
          ...
          <Button onClick={handleConfirmedAction} color="primary" variant="contained" autoFocus>
            Confirm
          </Button>
        </DialogActions>
      </Dialog>
