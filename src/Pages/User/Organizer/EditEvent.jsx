...
const MAX_IMAGES = 5
const API_BASE_URL = VITE_API_IMG_BASE_URL // or your actual API base URL
...
const EditEvent = () => {
  ...
  const normalizeFormData = (data) => {
    const safeDate = (val) => {
      ...
      try {
        ...
        return isNaN(d?.getTime()) ? "" : d.toISOString(); // Use ISO string for consistent comparison
      } catch {
        return "";
      }
    };
  ...
  useEffect(() => {
    ...
    if (currentForm.visibility != originalForm.visibility) {
  ...
  const prepareEventData = (status) => {
    ...
    // Format dates in UTC consistently
    ...
    if (formData.visibility === "public" && isVisibilityChanged) {
      eventViewService.getEventAttendeeLists(eventId)
        .then(response => {
          console.log('response:', response[0]);
          thenValueListID = response[0]?.Id;
          console.log('listIdforref:', thenValueListID);
        })
        .finally(() => {
          if (thenValueListID) {
            axios.delete(
              import.meta.env.VITE_API_BASE_URL + `/event/${eventId}/detach-attendee-list/${thenValueListID}`,
      ...
      if (formData.visibility === "private" && isVisibilityChanged) {

        eventViewService.getEventAttendeeLists(eventId).then(response => {
          eventService.deleteDefaultList(response[0].Id, eventId);
        });
      }
    }


    // Handle registration expiry - convert to UTC ISO string
    if (formData.registrationExpiry instanceof Date && !isNaN(formData.registrationExpiry)) {
      ...
      // Convert to UTC but preserve the time components
      ...
      formDataObj.append("RegistrationExpiry", utcExpiry.toISOString());
    } else {
                  ...
                  <div className="flex items-start space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-gray-700">
                        {/* // In your date display components, use this format: */}
                        {formData.date.toLocaleDateString("en-US", {
                          timeZone: 'UTC',  // Add this to display dates consistently
                          weekday: "long",
                          month: "long",
                          day: "numeric",
                          year: "numeric"
        ...
        <div className="w-full lg:w-1/3 space-y-6 lg:sticky lg:top-4 lg:self-start">
          ...
          <div className="space-y-4 p-5 bg-gray-50 rounded-xl border border-gray-100">
              ...
              <span className="text-sm">
                <p className="text-gray-700">
                  {formData.registrationExpiry && !isNaN(new Date(formData.registrationExpiry))
                    ? new Date(formData.registrationExpiry).toLocaleString("en-US", {
                      timeZone: "UTC", // Display in UTC consistently
                      weekday: "long",
                      month: "long",
                      day: "numeric",
                      year: "numeric",
            ...
            <div className="flex items-center justify-between">
              <span
                className={`px-2.5 py-1 text-xs rounded-full font-medium ${saveStatus === "draft" ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800"
                  }`}
              >
